...
import static com.aroundThirty.Resource.BR.temporaryListAll;
import static com.aroundThirty.Resource.BR.temporary_PageNum;
import static com.aroundThirty.Resource.FR.*;


public class TemporaryPagingBtn extends JPanel {
    ...
    public TemporaryPagingBtn() {
        if (temporaryListAll.size()%12 == 0){
            size = temporaryListAll.size()/12;
        } else if (temporaryListAll.size()%12 != 0){
            size = (temporaryListAll.size()/12) + 1;
        }
            ...
            btnList.get(i).addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    if (e.getSource() instanceof JButton) { // e.getsource로 받아온 객체가 JButton의 상속을 받으면 true 반환
                        ...
                        temporary_PageNum = (Integer.parseInt(btn.getText())-1);
                        temporary_StartIndex = SIZE_ITEM * temporary_PageNum;
